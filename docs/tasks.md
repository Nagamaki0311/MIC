...
| T-008 | 実機報告: 音声処理パイプライン・デフォルトプリセットの実運用品質不足(声の途切れ・遠い/小さい・過剰ノイズ抑制)の再検証・再設計 | 高 | 計画中 | claude | 実機テストで「あーーー」「もしもし」でも声が途切れる、十分な声量でも遠く/小さく聞こえる、デフォルトプリセット(quiet_low_voice)のnoise_stage.background_wet_dry_mixが1.0(MAX、RNNoise100%wet)になっている、との報告。Manager調査で仮説を特定(下記備考)、Plannerへ計画作成を委任 |
