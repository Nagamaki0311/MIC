...
| T-008 | 実機報告: 音声処理パイプライン・デフォルトプリセットの実運用品質不足(声の途切れ・遠い/小さい・過剰ノイズ抑制)の再検証・再設計 | 高 | 実装中 | claude | Plannerが構造的欠陥3点(RNNoise出力の1フレーム遅延によるdry/wetコムフィルタ、ゲートのヒステリシス欠如+完全ミュート+フレーム境界不連続、Compressorにmakeup gainが無くAGCの収束が遅すぎる)を特定。実測(Step0)→構造修正→プリセット再調整→16シナリオテストの実装計画を作成。Managerが承認事項を決定しDeveloperへ委任。詳細はdocs/decisions.md D-015参照 |
