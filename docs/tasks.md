...
| T-006 | ノイズ処理のバックグラウンド/インパクト2系統分離・最終総点検 | 高 | レビュー中 | claude | D-012の設計をそのまま実装(TransientDetector新設、NoiseStage2分割、14条件テスト追加)。Manager追加指摘2件(詳細設定スライダーのライブ反映UX、詳細設定パネルの縦スクロール対応)も対応済み。pytest 118 passed、pyflakes警告0件、bench/soak再測定も閾値内。詳細はdocs/decisions.md D-013参照 |
