...
| T-008 | 実機報告: 音声処理パイプライン・デフォルトプリセットの実運用品質不足(声の途切れ・遠い/小さい・過剰ノイズ抑制)の再検証・再設計 | 高 | レビュー中 | claude | Developerが実装完了。Step0実測でRNNoiseの入出力遅延は0サンプルと判明し(仮説だったdry/wet整列Stepは不要と確認、回帰テストで固定)、ゲートのフロア化+ヒステリシス+hangover、AGC時定数短縮(0.4s/1.5s)+フレーム内線形ランプ、プリセット値再調整(mix/EQ/Compressor)、GUIのAGCパラメータ欠落バグ修正、ジッタバッファpriming(Step7)まで実施。pytest 141 passed(既存123件相当+新規18件)、pyflakes警告0件。16シナリオテストのうち3件(もしもし反復・朗読・発声→無音)は修正前コードで実際に失敗し修正後にpassすることを確認(フレーム境界クリック検出)。Reviewerによる検証待ち。詳細はdocs/decisions.md D-015参照 |
